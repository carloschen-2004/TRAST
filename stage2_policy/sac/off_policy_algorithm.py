...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        super(OffPolicyAlgorithm, self).__init__(
            policy = policy,
            env = env,
            # policy_base = policy_base,  # 新版stable_baselines3不再接受此参数
            ...
            # create_eval_env = create_eval_env,  # 新版stable_baselines3不再接受此参数
            # monitor_wrapper = monitor_wrapper,  # 新版stable_baselines3不再接受此参数
            seed = seed,
            use_sde = use_sde,
        ...
        self.create_eval_env = create_eval_env
        self.monitor_wrapper = monitor_wrapper
    ...
    def _setup_model(self) -> None:
        ...
        # 从环境中获取 hidden_state_space（如果存在）
        if hasattr(self.env, 'envs'):
            ...
            if hasattr(env, 'hidden_state_space'):
                ...
                self.hidden_state_space = self.observation_space
        elif hasattr(self.env, 'hidden_state_space'):
            # 直接的环境
            self.hidden_state_space = self.env.hidden_state_space
        else:
            # 使用默认的 observation_space
            self.hidden_state_space = self.observation_space
    ...
    def _setup_learn(
        ...
        # 新版 stable_baselines3 的 _setup_learn 只接受部分参数
        # 我们需要手动设置一些参数
        self.eval_env = eval_env
        self.eval_freq = eval_freq
        self.n_eval_episodes = n_eval_episodes
        self.eval_log_path = log_path
